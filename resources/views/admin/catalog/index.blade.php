...
@section('page_id','admin.catalog.index')
          ...
          <div class="table-responsive">
            <table id="categoryTable" class="table table-bordered table-striped align-middle">
              <thead class="table-light">
                <tr>
                  <th class="checkAllWidth">
                    <input type="checkbox" data-check-all aria-label="Chọn tất cả danh mục">
                  </th>
              ...
              <tbody>
                ...
                <tr>
                  <td>
                    <input type="checkbox" data-row-check value="{{ $cat->id }}" aria-label="Chọn danh mục #{{ $cat->id }}">
                  </td>
                  ...
                  <td class="text-center">
                    ...
                    {{-- Xoá 1 danh mục: dùng FORM DELETE + data-confirm (không dùng <a> cho route DELETE) --}}
                    <form method="POST" action="{{ route('admin.categories.destroy', $cat->id) }}" class="d-inline" data-confirm="Xoá danh mục #{{ $cat->id }}?">
                      @csrf
                      @method('DELETE')
          ...
          <div class="table-responsive">
            <table id="brandTable" class="table table-bordered table-striped align-middle">
              <thead class="table-light">
                <tr>
                  <th class="checkAllWidth">
                    <input type="checkbox" data-check-all aria-label="Chọn tất cả NSX">
                  </th>
                  <th class="STT_Width">#</th>
                  <th>Tên</th>
                  <th>Slug</th>
              ...
              <tbody>
                ...
                <tr>
                  <td>
                    <input type="checkbox" data-row-check value="{{ $brand->id }}" aria-label="Chọn NSX #{{ $brand->id }}">
                  </td>
                  ...
                  <td class="text-center">
                    ...
                    {{-- Xoá 1 NSX: FORM DELETE + data-confirm --}}
                    <form method="POST" action="{{ route('admin.brands.destroy', $brand->id) }}" class="d-inline brandDeleteForm" data-confirm="Xoá NSX #{{ $brand->id }}?">
                      @csrf @method('DELETE')
...
{{-- Bulk forms --}}
<form id="catBulkForm" method="POST" action="{{ route('admin.categories.bulk-delete') }}" class="d-none" data-confirm="Xoá các danh mục đã chọn?">
  @csrf
  <div id="catBulkIds"></div>
</form>
<form id="brandBulkForm" method="POST" action="{{ route('admin.brands.bulk-delete') }}" class="d-none" data-confirm="Xoá các NSX đã chọn?">
  @csrf
