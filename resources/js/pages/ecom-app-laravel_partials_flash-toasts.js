...
/**
 * Khi quay lại bằng nút Back (back/forward navigation),
 * Chrome/Firefox dùng BFCache => DOM cũ (kèm toast) được restore.
 * Đoạn này sẽ xoá hết toast-container trong trường hợp đó.
 */
window.addEventListener('pageshow', function (event) {
  var entries = (window.performance && performance.getEntriesByType)
    ? performance.getEntriesByType('navigation')
    : [];
  var navEntry = entries && entries.length ? entries[0] : null;
  var isBackForward = event.persisted || (navEntry && navEntry.type === 'back_forward');

  if (!isBackForward) {
    return;
  }

  document.querySelectorAll('.toast-container').forEach(function (el) {
    el.remove();
  });
});
