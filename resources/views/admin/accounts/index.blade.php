...
@section('page_id','admin.accounts.index')
          ...
          <div class="table-responsive">
            <table id="accountTable" class="table table-bordered table-striped align-middle">
              <thead class="table-light">
                <tr>
                  <th class="checkAllWidth">
                    <input type="checkbox" data-check-all aria-label="Chọn tất cả">
                  </th>
                  <th>#</th>
                  <th>Tên</th>
                  <th>Email</th>
              ...
              <tbody>
                ...
                <tr>
                  <td>
                    <input type="checkbox" data-row-check value="{{ $acc->id }}" aria-label="Chọn bản ghi #{{ $acc->id }}">
                  </td>
                  ...
                  <td>{{ $acc->phone ?? '-' }}</td>
                  <td>
                    @if($acc->roles->isNotEmpty())
                    {{ $acc->roles->pluck('name')->join(', ') }}
                    @else
                  ...
                  <td class="text-center">
                    <button type="button"
                      ...
                      data-update-url="{{ route('admin.accounts.update', $acc->id) }}"
                      data-full_name="{{ $acc->full_name }}"
                      data-email="{{ $acc->email }}"
                      data-phone="{{ $acc->phone }}"
                      data-address="{{ $acc->address }}"
                      data-status="{{ $acc->status }}"
                      data-avatar="{{ $acc->avatar ? Storage::url($acc->avatar) : Storage::url('avatars/base-avatar.jpg') }}"
                      data-role_ids="{{ $acc->roles->pluck('id')->join(',') }}">
