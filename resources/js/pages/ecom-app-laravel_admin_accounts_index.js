...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function refreshMaster() {
    ...
    const c = rowCbs();
    const t = c.length;
    const k = c.filter(x => x.checked).length;
    master.indeterminate = false;
    master.checked = t > 0 && k === t;
  }
  ...
  if (btnOpen) {
    btnOpen.addEventListener('click', async () => {
      ...
      if (!ids.length) {
        await UIConfirmWrap({ title: 'Thiếu lựa chọn', message: 'Vui lòng chọn ít nhất <b>1</b> tài khoản.' });
        return;
      }

      if (val !== 'ACTIVE' && val !== 'BAN') {
        await UIConfirmWrap({ title: 'Chưa chọn trạng thái', message: 'Vui lòng chọn trạng thái đích.' });
        return;
      }

      const ok = await UIConfirmWrap({
        title: 'Xác nhận cập nhật',
        message: 'Bạn sắp cập nhật cho <b>' + ids.length + '</b> tài khoản.<br>Trạng thái: <span class="badge ' + (val === 'ACTIVE' ? 'bg-success' : 'bg-danger') + '">' + val + '</span>'
      });
      if (!ok) return;
  ...
  function renderRoles() {
    ...
    if (selected.length === 0) {
      ...
      tokensBox.innerHTML = selected.map(id => {
  ...
  if (tokensBox) {
    tokensBox.addEventListener('click', (e) => {
      const id = e.target && e.target.getAttribute ? e.target.getAttribute('data-x') : null;
      if (id) removeById(id);
    });
  }
  ...
  if (suggestEl) {
    suggestEl.addEventListener('click', (e) => {
      const chip = e.target.closest ? e.target.closest('[data-add]') : null;
  ...
  function fillFormFromBtn(btn) {
    ...
    const upd = document.getElementById('__update_action');
    if (upd) upd.value = url;
  ...
  function setActiveTab(tab) {
    ...
    document.querySelectorAll('.admin-accounts-nav-link').forEach(link => link.classList.remove('active'));
  ...
  function setupTabs() {
    document.querySelectorAll('.admin-accounts-nav-link').forEach(link => {
